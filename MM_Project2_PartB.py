...
This program implements the Minimax algorithm to choose the best move for a Tic-Tac-Toe computer player.
...
class Game:
    ...
    def minimax(self, maximizing):
        # Uses the function get_winner to determine if there is a winner and if so who it is
    ...
    def minimax_play(self):
        best_score, best_move = self.minimax(True)
        return best_move
    ...
    def playGame(self):
        ...
        while True:
            ...
            best_move = self.minimax_play()
            print(f"Computer chooses row #{best_move[0]} and column #{best_move[1]}")
